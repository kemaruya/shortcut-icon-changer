...
- **約 300 種のスターターアイコンを同梱** — ネット接続なしですぐ選べます
- **カテゴリ（スタイル）と色調のタグクラウドで絞り込み** — ピッカー上部のタグをクリックして ON にすると一致するアイコンだけを表示（同種タグは OR、種別をまたぐと AND）。件数の多いタグほど大きく表示。キーワード検索ボックスも併用可
