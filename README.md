...
> 状態: **Phase 1 リリース済み**（v0.4.0・Windows 11 標準機能のみ・追加ランタイム/ビルド/署名は不要）／**Phase 2 ネイティブアプリ + MSI**（v0.8.0・ホーム画面/設定/ダークテーマ対応・下記参照）／**Windows 11 モダン コンテキスト メニュー対応**（v0.7.0〜・MSI のオプトイン・自己署名スパース MSIX）／**Microsoft Store 配布を準備中**（完全 MSIX・Store が再署名）
...
- **ホーム画面・設定・ダークテーマ・スプラッシュ最適化（v0.8.0）**: スタート メニューから起動できる**ホーム画面**（「アイコンを変更」「設定」の 2 カード）と**設定画面**（言語・テーマ切り替え）を追加しました。アプリ全体で**ライト / ダーク / システム追従のテーマ**に対応します。右クリック ピッカーには**対象バー**（変更対象の `.lnk` 名と現在のアイコンを表示）を設け、ホームから開いた場合は対象を未選択であることを明示します。起動高速化にあわせてスプラッシュを**遅延ゲート式**に変更し、ホーム経由では表示せず、右クリックの初回コールド時に描画が遅い場合だけ一瞬表示します。さらに **Microsoft Store 配布用の完全（自己完結）MSIX** ビルドを追加しました（Store がアップロード後に信頼された証明書で再署名するため自己署名は不要）

### インストール（MSI）

[Releases](https://github.com/kemaruya/shortcut-icon-changer/releases) から `ShortcutIconChanger-X.Y.Z-perUser.msi` を入手し、ダブルクリック（またはサイレント `msiexec /i ShortcutIconChanger-0.8.0-perUser.msi /qn`）でインストールします。管理者権限・UAC は不要です。`%LOCALAPPDATA%\Programs\ShortcutIconChanger` に導入され、`.lnk` の右クリックメニューに「アイコンを変更」が登録されます。

アンインストールは「アプリと機能」から、またはサイレントに `msiexec /x ShortcutIconChanger-0.8.0-perUser.msi /qn` で行えます。
